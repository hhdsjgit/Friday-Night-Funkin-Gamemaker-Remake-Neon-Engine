...
function draw_Outline_s(text,x,y,xscale,yscale,color,color1,angle,font){
	//描边绘制字体
	draw_set_font(Font_vcr)
	//检查字体是否存在
	if font_exists(font){
	...
	//设置颜色
	draw_set_colour(color);
	draw_text_transformed(x+1, y-1, text_str,xscale, yscale, angle);
	draw_text_transformed(x+1, y, text_str, xscale, yscale, angle);
	draw_text_transformed(x-1, y+1, text_str, xscale, yscale, angle);
	draw_text_transformed(x-1, y, text_str, xscale, yscale, angle);
	draw_text_transformed(x, y+1, text_str, xscale, yscale, angle);
	draw_text_transformed(x, y+1, text_str, xscale, yscale, angle);
	draw_text_transformed(x, y-1, text_str, xscale, yscale, angle);
	draw_text_transformed(x-1, y-1, text_str, xscale, yscale, angle);
